"""
fisher_forecast_noz2p2.py

Fisher matrix forecast for detecting the Union3 DDE spectral tilt
EXCLUDING the z=2.2 redshift bin (min_z=2.4).

Results saved to dde_analysis/results/no_z2p2/
"""
...
print("="*60)
print(f"Fisher forecast — {LABEL} (min_z={MIN_Z}, z=2.2 excluded)")
print("="*60)
...
ax.loglog(vols, SN * np.sqrt(vols),
          color='steelblue', linewidth=2.5, label=f'S/N ({LABEL})')
...
plt.suptitle(f'Fisher forecast — {LABEL} (z=2.2 excluded)',
             fontsize=13, y=1.01)
plt.tight_layout()
...
with open(outfile, 'w') as f:
    ...
    f.write("="*50 + "\n")
    f.write(f"min_z = {MIN_Z}  (z=2.2 excluded)\n")
